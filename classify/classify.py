...
# Specify the aspect to classify. 1: Structure 2: Information 3: Wording
aspect = 1

# Specify the algorithm. 0: Bi-LSTM 1: Random Forest 2: Decision Tree 3: Logistic Regression 4: SVM
ml_algorithm = 1
...
path = 'input/message_input.csv'
model_location = 'model'  +'/model'
embedding_location = 'embedding' + '/Word2vec_model' + '.pkl'
...
def train_model_ml(x_train, x_test, y_train, y_test):
    ...
    if ml_algorithm == 1:
        ...
        clf = RandomForestClassifier().fit(x_train,y_train)
        print("Invalid input, using Random Forest Classifier instead")
...
def StratifiedKFold_train():
    data_full=pd.read_csv(path, usecols=[0,1,2,3,4,5,6,7], engine='python') 
    dataset = data_full.values
    if aspect in [1,2,3]:
        aspect_column = aspect + 3
    else:
        aspect_column = 5
    classes = dataset[:, aspect_column]
